#include <iostream>
#include <vector>
#include <map>

using namespace std;

vector<pair<int,int>> v;
map<pair<int, long long>, int> cache;

int solve(int cur, long long bits) {
    if (cache.find(make_pair(cur, bits)) != cache.end())
        return cache[make_pair(cur, bits)];
    int ans = 0;
    for (int i = 0; i < v.size(); ++i) {
        if ((1LL<<i) & bits) continue;
        auto p = v[i];
        const int to_add = p.first + p.second;
        if (p.first == cur) ans = max(ans, to_add + solve(p.second, bits | (1LL << i)));
        if (p.second == cur) ans = max(ans, to_add + solve(p.first, bits | (1LL << i)));
    }
    cache[make_pair(cur, bits)] = ans;
    return ans;
}

int main() {
    int N;
    cin >> N;
    for (int i = 0; i < N; ++i) {
        int a, b;
        cin >> a >> b;
        v.push_back(make_pair(a,b));
    }

    cout << solve(0, 0) << endl;
}
